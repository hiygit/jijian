  ...
  <div class="content home">
    ...
    <div class="StandBook" v-loading="loading">
      ...
      <div class="showDataTable">
        <div class="formSearch" style="margin-bottom: 30px;">
          <span style="color: #fff;font-size: 1.09375vw;">查找：</span>
        ...
        <el-table v-loading="loading1" :data="tableData" border :height="tableHeight" stripe style="background-color:transparent" :cell-style="cellStyle" :header-cell-style="headerCellStyle" width="100%">
          <el-table-column :resizable="false" prop="number" label="序号" type="index" :index="indexinfo" width="80"></el-table-column>
          <el-table-column :resizable="false" prop="alwaysTime" label="来件时间"></el-table-column>
          <!-- <el-table-column prop="createDate" label="收件时间"></el-table-column> -->
          <el-table-column :resizable="false" prop="clueStoTheSource" label="线索来源"></el-table-column>
          <el-table-column :resizable="false" prop="groupNumber" label="组内线索编号"></el-table-column>
          <el-table-column :resizable="false" prop="clueNumber" label="线索件数"></el-table-column>
          <el-table-column :resizable="false" prop="isDuplicateOrNot" label="是否重复件"></el-table-column>
          <el-table-column :resizable="false" prop="transferredResult" label="办理结果"></el-table-column>
          <el-table-column :resizable="false" prop="disponsalWay" label="进度"></el-table-column>
          <el-table-column :resizable="false" prop="name" label="被反映人姓名"></el-table-column>
          <el-table-column :resizable="false" prop="office" label="被反映人工作单位" width="130px"></el-table-column>
          <el-table-column :resizable="false" prop="presentOccupation" label="被反映人职务"></el-table-column>
          <el-table-column :resizable="false" label="操作" width="360px">
...
export default {
  ...
  methods: {
    ...
    deletebtn(indx,item){
      ...
      this.$ajax({
        method: "POST",
        url: this.globals.requesturl + 'web/cluesparameter/delete',
        data: objstr,
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
    ...
    searckkeyword() {
      ...
      if(that.startDate == "" && that.endDate == ""){
        ...
        this.$ajax({
          ...
          if (res.data.success && res.data.errorCode == "-1") {
            let tablearr = [];that.loading1=false;
              ...
              for (let w = 0; w < res.data.body.list.length; w++) {
                for (let i = 0; i < res.data.body.list[w].cluesParameterList.length; i++ ) {
                  ...
                  let obj = {
                    ...
                    transferredResult: lists.transferredResult,
                    disponsalWay: lists.disponsalWay,
                    name: lists.personnel != undefined ? lists.personnel.name : "",
                    office: lists.personnel != undefined ? lists.personnel.office.name : "",
                    presentOccupation: lists.personnel !=  undefined ? lists.personnel.presentOccupation : "",
                    id: lists.id //线索台账id
                  };
                  for (let s = 0; s < that.sourcelist.length; s++) {
                    if (that.sourcelist[s].value == lists.clueStoTheSource) {
...
<style scoped>
  .StandBook /deep/ .el-col-18 {
      width: 84%;
  }
  .StandBook /deep/ .el-col-6 {
      width: 16%;
  }
  .StandBook /deep/ .el-input--prefix .el-input__inner{
    background: none;
    width: 85%;
    height: 2.8vw;
    border: 1px solid rgba(3, 208, 255, 0.7);
  }
  .StandBook /deep/ .el-date-editor.el-input{
    width: 15.1875vw;
    margin-right: -15px;
  }
  .StandBook /deep/ .el-input__inner{
    color: #fff !important;
    margin-top: 3px;
  }
  .StandBook /deep/ .el-input__suffix{
    right: 3vw !important;
  }
</style>
