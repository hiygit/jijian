  ...
  <div class="xiansuotaizhang">
    ...
    <div class="navcrumbed">
      ...
      <div class="navcrumbedlist">
        <el-breadcrumb separator-class="el-icon-arrow-right">
          <div @click="toStandBook" style="display: inline-block;float:left;cursor:pointer;">线索台账&nbsp;<span style="color:#BAC4CA"> > </span> </div>
    ...
    <div class="content home">
      <div class="StandBookEnter">
        ...
        <!-- 按钮 -->
        <div class="bottomSub">
          <button class="bottomBtnStyle" @click="savebtns('msgform')" v-if="!updata" style="background:#e6a23c;">保存</button>
          ...
          <div class="personnelframe-container-box">
            ...
            <div class="tablebox">
              ...
              :cell-style="cellStyle" :header-cell-style="headerCellStyle" width="100%" @row-click="singleElection">
                <el-table-column label :resizable="false" width="65">
                  <template slot-scope="scope">
                    <el-radio class="radio" v-model="templateSelection" :label="scope.$index">&nbsp;&nbsp;</el-radio>
                  </template>
                </el-table-column>
                <el-table-column label="序号" type="index" :resizable="false" :index="indexMethod" width="100"></el-table-column>
                <el-table-column label="姓名" prop="name" :resizable="false" width="100"></el-table-column>
                <el-table-column label="单位" prop="officename" :resizable="false" width="300"></el-table-column>
                <el-table-column label="现任职务" prop="presentOccupation" :resizable="false" width="200"></el-table-column>
              </el-table>
            </div>
...
export default {
  ...
  mounted() {
    ...
    if (this.$route.query.isactive == 0 && this.$route.query.radio == 2 || this.$route.query.type == 3 || this.$route.query.type == 4){
      this.isOutOffice = 1
    } else if (this.$route.query.isactive == 1 && this.$route.query.radio == 2 || this.$route.query.type == 3 || this.$route.query.type == 4) {
      this.isOutOffice = 1
    } else {
      this.isOutOffice = 0
    }
  ...
  methods: {
          ...
          if (res.data.success && res.data.errorCode == '-1') {
            ...
            if (res.data.body.type == 1 || res.data.body.type == 3 || res.data.body.type == 4) {
              let obj = {
                ...
                officeName: res.data.body.personnel.officeName == undefined ? res.data.body.personnel.externalPersonnelUnit: res.data.body.personnel.officeName, //工作单位
              ...
              console.log(res.data.body.personnel)
                ...
                for (let i = 0; i < cluesParameterChildrenList.length; i++) {
                  ...
                  if(list.personnel != undefined && list.personnel != ""){
                    let obj = {
                      ...
                      officeName: res.data.body.beReflectedUnitName, //工作单位
    ...
    getPersonnelList(item, index) {
      ...
      if(this.$route.query.isactive == 1 && this.$route.query.radio == 1 || this.$route.query.type == 2){
      if(this.personnelOfficeName == "" && this.personnelOfficeId == ""){
        this.$message.warning("请填写被反映人单位")
      }else{
        this.$ajax({
          method: 'POST',
      ...
      this.$ajax({
        ...
        if (res.data.success && res.data.errorCode == '-1') {
          ...
          if (res.data.body.personnel == '') {
            ...
            if (res.data.body.personnel.officeName == undefined) {
    ...
    // 页面跳转 查询弹窗新增
    goLink(e) {
      if(this.msgform.clueStoTheSource){
        this.personnelpops1 = false //查询弹窗
        localStorage.setItem('data', JSON.stringify(this.$data))
      }else{
        this.msgform.clueStoTheSource = this.msgform.clueStoTheSource
      }
      if (this.find.reflectunitid && typeof this.find.reflectunitid != 'string') {
        let objs = {
          userId: JSON.parse(localStorage.getItem('userinfo')).isd,
        ...
        this.$ajax({
          ...
          res.data.body.officeList.forEach(item => {
            if (item.id == this.find.reflectunitid[this.find.reflectunitid.length - 1]) {
      ...
      if (this.personnelOfficeId && typeof this.personnelOfficeId != 'string') {
        let objs = {
          userId: JSON.parse(localStorage.getItem('userinfo')).isd,
          parentId: this.personnelOfficeId[this.personnelOfficeId.length - 1]
        }
        this.$ajax({
          method: 'POST',
          url: this.globals.requesturl + 'web/office/getChildren',
          ...
          res.data.body.officeList.forEach(item => {
            if (item.id == this.personnelOfficeId[this.personnelOfficeId.length - 1]) {
